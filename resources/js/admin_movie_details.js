/**
 * resources/js/movie_cinema_formation.js
 *
 * TGV-style date/showtime renderer for Admin.
 * Replicates the Branch Manager mechanism identically.
 */
(function () {
    'use strict';

    var dataEl = document.getElementById('mcf-data');
    if (!dataEl) return;

    var dateGroups = JSON.parse(dataEl.dataset.dates || '[]');
    if (dateGroups.length === 0) return;

    var dateStrip       = document.getElementById('mcf-date-strip');
    var showtimeSection = document.getElementById('mcf-showtime-section');

    if (!dateStrip || !showtimeSection) return;

    /* ── Build date strip ─────────────────────────────────── */
    function buildDateStrip() {
        dateGroups.forEach(function (group, idx) {
            var btn = document.createElement('button');
            btn.type        = 'button';
            btn.className   = 'mcf-date-btn';
            btn.dataset.idx = idx;

            var dayEl = document.createElement('span');
            dayEl.className   = 'mcf-date-btn__day';
            dayEl.textContent = group.label_day;

            var numEl = document.createElement('span');
            numEl.className   = 'mcf-date-btn__num';
            numEl.textContent = group.label_num;

            var monthEl = document.createElement('span');
            monthEl.className   = 'mcf-date-btn__month';
            monthEl.textContent = group.label_month;

            btn.appendChild(dayEl);
            btn.appendChild(numEl);
            btn.appendChild(monthEl);

            btn.addEventListener('click', function () {
                // Update active state
                dateStrip.querySelectorAll('.mcf-date-btn').forEach(function (b) {
                    b.classList.remove('mcf-date-btn--active');
                });
                btn.classList.add('mcf-date-btn--active');

                renderShowtimes(group);
            });

            dateStrip.appendChild(btn);
        });
    }

    /* ── Render theatres + time pills for a given date group ─ */
    function renderShowtimes(group) {
        showtimeSection.innerHTML = '';

        if (!group.theatres || group.theatres.length === 0) {
            var empty = document.createElement('p');
            empty.style.cssText = 'color:var(--text-muted);font-size:0.82rem;padding:16px 0;';
            empty.textContent   = 'No showtimes available for this date.';
            showtimeSection.appendChild(empty);
            return;
        }

        group.theatres.forEach(function (theatre) {
            // Theatre block wrapper
            var block = document.createElement('div');
            block.className = 'mcf-theatre-block';

            // Theatre name heading
            var nameEl = document.createElement('div');
            nameEl.className   = 'mcf-theatre-name';
            nameEl.textContent = theatre.name;
            block.appendChild(nameEl);

            // Time pills row
            var pillsWrap = document.createElement('div');
            pillsWrap.className = 'mcf-time-pills';

            if (!theatre.times || theatre.times.length === 0) {
                var hint = document.createElement('span');
                hint.style.cssText = 'font-size:0.78rem;color:var(--text-muted);';
                hint.textContent   = 'No times available.';
                pillsWrap.appendChild(hint);
            } else {
                theatre.times.forEach(function (time) {
                    var pill = document.createElement('button');
                    pill.type        = 'button';
                    pill.className   = 'mcf-time-pill';
                    pill.textContent = time;
                    // Placeholder click
                    pill.addEventListener('click', function () {
                        // future logic
                    });
                    pillsWrap.appendChild(pill);
                });
            }

            block.appendChild(pillsWrap);
            showtimeSection.appendChild(block);
        });
    }

    /* ── Init ─────────────────────────────────────────────── */
    buildDateStrip();

    // Auto-select first date button
    var firstBtn = dateStrip.querySelector('.mcf-date-btn');
    if (firstBtn) firstBtn.click();

})();